...
class EditStore extends Component {
  ...
  handleChange = name => event => {
    const value = name === "image"
      ...
      : event.target.value
    this.storeData.set(name, value)
    this.setState({ [name]: value })
  }

  render() {
    const logoUrl = this.state.id
          ? `/api/stores/logo/${this.state.id}?${new Date().getTime()}`
          : "/api/stores/defaultphoto"
    if (this.state.redirect) {
      return (<Redirect to={"/seller/stores"}/>)
    }
    ...
    return (<div className={classes.root}>
      <Grid container spacing={24}>
        <Grid item xs={6} sm={6}>
          ...
          <Grid item xs={6} sm={6}>
            <MyProducts storeId={this.match.params.storeId}/>
          </Grid>
        </Grid>
    </div>)
  }
...
export default withStyles(styles)(EditStore);
